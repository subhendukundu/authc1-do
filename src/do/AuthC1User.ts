...
import { expiredOrInvalidCode, handleError } from "../utils/error-responses";
...
export class AuthC1User implements DurableObject {
  ...
  constructor(
    ...
    this.state.blockConcurrencyWhile(async () => {
      const userData = await this.state.storage?.get<UserData>("userData");
      if (userData) {
        this.userData = userData || {};
      }
      const apps = await this.state.storage?.get<Record<string, AccessedApp>>(
        "apps"
      );
      if (apps) {
        this.apps = apps;
    ...
    this.app.patch("/sessions/:sessionId", async (c: Context) => {
      const sessionId = c.req.param("sessionId");
      const sessionData = (await c.req.json()) as SessionData;
    ...
    this.app.patch("/verify/:sessionId", async (c: Context) => {
      ...
      await Promise.all([
        this.state.storage?.put("userData", this.userData),
        this.state.storage?.put("sessions", this.sessions),
      ]);
    ...
    this.app.post("/refresh", async (c: Context) => {
      ...
      const { email, emailVerified } = this.userData;
  ...
  async clearSessionsByExpiration() {
    ...
    Object.entries(allSessions).forEach(([sessionId, sessionData]) => {
      const sessionExpirationTime = new Date(sessionData.createdAt).getTime();
...
export class UserClient {
  ...
  async createSession(appSettings: ApplicationRequest): Promise<AuthResponse> {
